...
dataMag_tot_dB = dataMag_tot_dB + abs(min(dataMag_tot_dB));
% dataMag_int_dB = 10.*log10(dataMag_int);
% dataMag_tot_dB(dataMag_tot_dB<-40) = -40;
% h = polarplot(azi,dataMag_tot_dB+40,'Linewidth',2,'color',[0 0 1]);
% haxes = get(h,'Parent');
% haxes.RTickLabel = {'-40','-30','-20','-10','0'};
% title('Total Radiation Pattern at 9GHz');

dataMag_tot_dB = 10.*log10(smooth(dataMag_int(:,2)));
dataMag_tot_dB = dataMag_tot_dB + abs(min(dataMag_tot_dB));

% Plot measured single patch
ticks = linspace(0,max(array),5);
polarplot(azi,dataMag_tot_dB.*3,'Linewidth',5);
rticks(ticks)
...
% ticks = linspace(0,max(array),5);
% polarplot(theta+pi,array,azi,dataMag_tot_dB.*2,'Linewidth',5);
% rticks(ticks)
% rticklabels({'\bf{-26.22}','\bf{-16.22}','\bf{-6.22}','\bf{3.78}','\bf{13.78}'})
% title('\phi-Cut Radiation Pattern at 9GHz (dB)','FontSize',20);
% legend('WIPL-D Simulated 5-Element Array','ATR Measured 5-Element Array','FontSize',30);

% Plotting array only
% ticks = linspace(0,max(array),5);
% % polarplot(azi,smooth(dataMag_tot_dB.*2),'Linewidth',5);
% polarplot(theta+pi,array,'Linewidth',5);
% rticks(ticks)
% rticklabels({'\bf{-26.22}','\bf{-16.22}','\bf{-6.22}','\bf{3.78}','\bf{13.78}'})
% title('\phi-Cut Radiation Pattern of WIPL-D Simulated at 9GHz (dB)','FontSize',20);
